...
## 📅 Session log — 2026-06-06b (For You standalone screen shell + CW "Both" filter)

### Headline

Added a new top-level **For You** navigation screen (pure-rows canvas, no hero)
+ the missing **"Both"** Continue-Watching filter. Additive; CW pipeline guarded.
Installed + smoke-tested on the Jawwy TV; later same-day follow-up fixed two
cosmetic issues (distinct CW row titles + first-row clears the TopBar).

### PART 1 — CW "Both" filter

`ContinueWatchingFilter.BOTH`; `LayoutRowKind.CONTINUE_WATCHING` → BOTH (was
Series); `forContinueWatchingBoth()` reuses the legacy `"continue_watching"` id;
"+ Continue Watching" submenu gains **Both**. The legacy CONTINUE_WATCHING→SERIES
migration was made **one-shot + HOME-scope-only** (gated by `cw_split_seeded`) so
user-added Both rows survive.

### PART 2 — For You shell (additive)

`LayoutScreenScope.FOR_YOU` + `CategoryPill.FOR_YOU` (first pill) + `Screen.ForYou`
+ `ForYouScreen`/`ForYouViewModel`. Reuses HomeScreen wholesale; FOR_YOU forced to
**Classic + hero-off** via per-scope datastore defaults. Default seed (Trakt-authed
only): CW Both + Up Next. Default tab: authed → For You is the start destination
(+ one-shot `promoteForYouToFrontOnce()`); else Home. For You scope appears in the
Rows Manager (excluded from the layout-picker).

### Cosmetic follow-on

`ContinueWatchingSection` gained a `title: String? = null` param; ClassicHomeContent
passes each CW row's configured name (distinct titles). Classic reserves
`LocalTopBarOverlayHeight` as top padding when there's no hero so the first row
clears the TopBar.

### Build / deploy

`59d1b136` (shell + Both), `05e3772a` (cosmetic fixes). Installed + verified on the
Jawwy TV (`192.168.8.187`); no crashes.
...
- **New `core/trakt/TraktHomeCatalogResolver.kt`** (modeled on
  ...
  **profile-keyed** TTL cache (`"<profileId>:<kind>"` — no cross-profile leak),
  auth-gated, returns stale-but-present row on a transient blip, maps Trakt
  show/movie DTOs → `MetaPreview` via the shared `TraktImageUtils` helpers +
  `normalizeContentId`. No extra image fetching (uses Trakt's
...
- **CW data-quality investigation:** stale CW = (a) abandoned `<80%` Trakt
  ...
  Up Next (metadata-derived, no Trakt/TMDB `status` guard). Scrobbling works on
  the happy path.
