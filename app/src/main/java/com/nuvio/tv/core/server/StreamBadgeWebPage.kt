...
object StreamBadgeWebPage {
    fun html(rawContext: Context?): String {
        ...
        val appName =  "NuvioTV"
        val title =  "Fusion Style"
        val urlsTitle =  "Fusion badge URLs"
        val badgePositionTitle =  "Badge position"
        val badgePositionDescription =  "Choose whether Fusion and size badges appear above or below stream cards."
        val badgePositionTop =  "Top"
        val badgePositionBottom =  "Bottom"
        val description = "Import up to $STREAM_BADGE_IMPORT_LIMIT Fusion-style stream badge JSON URLs. Each URL can be updated or deleted separately."
        val urlLabel =  "Fusion badge JSON URL"
        val importAction =  "Import"
        val closeEmpty =  "No Fusion badge URLs imported."
        val active =  "Active"
        val inactive =  "Inactive"
        val preview =  "Preview"
        val delete =  "Delete"
        val select =  "Select"
        val previewTitle =  "Fusion badge preview"
        val previewEmpty =  "No Fusion-style badge images in this URL."
        val otherBadges =  "Other Fusion badges"
        val importing =  "Importing…"
        val imported =  "Imported badge URL."
        val importError =  "Badge import failed."
        val saved =  "Saved. New streams will use these settings."
        val deleted =  "Deleted badge URL."
        val saveError =  "Could not save"
        val loadError =  "Could not load stream badge settings"
        val summaryTemplate =  "%1\$d/%2\$d URLs, %3\$d active Fusion badges"
        val groupTitleTemplate =  "Group %1\$d"
        val statusSummaryTemplate =  "%1\$s, %2\$d enabled badges, %3\$d groups"
        val previewCountTemplate =  "%1\$d Fusion-style badges from this URL"
        return """
<!DOCTYPE html>
<html lang="en">
<head>
